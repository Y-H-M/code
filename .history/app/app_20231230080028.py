...
def read():
    ...
    with open("data.csv", mode="r", encoding="utf-8") as csv_data:
...
def plot():
    ...
    with open("n_workoutdata.csv", mode="r", encoding="utf-8") as csv_data:
        # Line Graph
        dates = []
        y_mood = []
        y_productivity = []
...
def write():
    """Calls ask() to get input from user, then updates the csv and outputs the plot"""
    with open("n_workoutdata.csv", mode="a", encoding="utf-8", newline="") as csv_data:
        if csv_data.tell() == 0:
