...
def plot():
    with open('app/data.csv', mode='r', encoding='utf-8') as csv_data:
        ...
        wworkoutimg, hworkoutimg = workoutimg.size
        
        wtotalimg = max(wgraphimg, wgoodimg, wworkoutimg)
        htotalimg = hgraphimg+ hgoodimg+ hworkoutimg
         
        totalimg = img.new("RGB", (wtotalimg, htotalimg), (255, 255, 255))
        
        totalimg.paste(wgraphimg, (0, 0))
        totalimg.paste(wgoodimg, (0, height1))
        totalimg.paste(wworkoutimg, (0, height1 + height2))
